...
void format(int Mode){
	FILE *fp;
	if(Mode==-2) fp=fopen("Mode 1.txt","w");
	if(Mode==0) fp=fopen("Mode 2.txt","w");
	if(Mode==2) fp=fopen("Mode 3.txt","w");
	if(Mode==4) fp=fopen("Mode 4.txt","w");
	if(Mode!=-2) fprintf(fp,"0\n1\n1\n0\n1\n0\n0\n1\n0\n0\n4\n0");
	else fprintf(fp,"0\n1\n1\n0\n1\n0\n0\n1\n0\n0\n4\n0");
	fclose(fp);
	if(Mode<=0) return;
	FILE *f;
	if(Mode==2) f=fopen("Item 1.txt","w");
	if(Mode==4) f=fopen("Item 2.txt","w");
	fprintf(f," ");
	fclose(f);
	if(Mode==2) f=fopen("Ban 1.txt","w");
	if(Mode==4) f=fopen("Ban 2.txt","w");
	fprintf(f," ");
	fclose(f); 
}
...
void BackUp(int *stage,int *Level,int *EXP,int *EggHealth,int*original,int Mode,int*stone,int*wlevel,int*gotEXP,int*gotStone,int*percento){
	...
	FILE *fp;
	if(Mode==-2) fp=fopen("Mode 1.txt","r");
	if(Mode==0) fp=fopen("Mode 2.txt","r");
	if(Mode==2) fp=fopen("Mode 3.txt","r");
	if(Mode==4) fp=fopen("Mode 4.txt","r");
	...
	*percento=atoi(percento1);
	...
	FILE *f;
	if(Mode==2) f=fopen("Item 1.txt","r");
	if(Mode==4) f=fopen("Item 2.txt","r");
	int i=0;
	for(i=0;i<iid;i++){
		memset(item1,0,10);
		fscanf(f,"%s",item1);
	...
	fclose(f);
	if(Mode==2) f=fopen("Ban 1.txt","r");
	if(Mode==4) f=fopen("Ban 2.txt","r");
	for(i=0;i<iid;i++){
		memset(ban1,0,10);
		fscanf(f,"%s",ban1);
		ban[i]=atoi(ban1);
...
int keyControl(int Stage,int wLevel,int answer,int Health,int *ich,int Mode,int ttime,int *ic){
	...
	if(t==9 && Mode>=2){
		...
		system("mode con cols=60 lines=40 | title 알 까기");
		if(aitem==1) { 
			PlaySound("TicTok.wav", NULL, SND_ASYNC);
			return Health/2;
		}
		else if(aitem==2){
			ich[(*ic)++]=2;
			PlaySound("Freeze.wav",NULL, SND_ASYNC);
		} 
		else if(aitem==3) {
			ich[(*ic)++]=3;
	...
	if(t==13){
		...
		Evaluate(1);
		if(aitem==0) {
			PlaySound("ItemUse.wav", NULL, SND_ASYNC | SND_FILENAME);
		}
...
void Evaluate(int p){
	if(p) {
		...
		printf("Perfect!!!");
		PlaySound("Correct.wav", NULL, SND_ASYNC);
		textcolor(WHITE,BLACK); 
	}
	else {
		going(23,23);
		...
		printf("Oops!");
		PlaySound("Oops.wav", NULL, SND_ASYNC);
		textcolor(WHITE,BLACK);
	}
}
...
int weapon(int*stone,int*wlevel,int EXP,int Level,int Stage,int Mode,int ttime,int percento,int sssstone,int wwwlevel,int gotEXP,int gotStone,int EggHealth, int original){
	...
	if(*stone>=t) {
		...
		weaponDraw(1);
		for(i=0;i<2;i++){
			PlaySound("anvil.wav",NULL,SND_ASYNC);
			wEffect(1,7+i);
			wEffect(1,8+i);
			Sleep(500);
		}
		...
		if(p==0){
			PlaySound("XP.wav",NULL,SND_ASYNC);
		}
		else {
			PlaySound("anvilB.wav",NULL,SND_ASYNC);
		}
...
int startGame(int Level,int Stage,int EXP,int Mode,int ttime,int percento,int EggHealth,int original,int stone,int wlevel,int gotEXP,int gotStone){
	...
	while(1 && EggHealth>0){
		...
		if(atk>0) {
			Count(Stage-1,Level,EXP,EggHealth,original,Mode,stone,wlevel,gotEXP,gotStone,percento,ttime);
			PlaySound("Touch.wav", NULL, SND_ASYNC | SND_FILENAME|SND_NOSTOP);
		}
	}
...
void Count(int stage,int Level,int EXP,int EggHealth,int original,int Mode,int stone,int wlevel,int gotEXP,int gotStone,int percento,int ttime){
	char touch[10]={};
	FILE *fp1;
	if(Mode==-2) fp1=fopen("Mode 1.txt","r");
	if(Mode==0) fp1=fopen("Mode 2.txt","r");
	if(Mode==2) fp1=fopen("Mode 3.txt","r");
	if(Mode==4) fp1=fopen("Mode 4.txt","r");
	fscanf(fp1,"%s",touch);
	fclose(fp1);
...
int Broken(int EggHealth,int original,int music,int Stage,int atk,int Level){
	...
	if(music!=mun && Stage%3!=0 && Stage%5!=0 && atk==Level*2) PlaySound("Broken.wav", NULL, SND_ASYNC);
	switch(mun){
		case 8:{
			going(7,14);
			printf("               **      *  **	     \n");
...
int changeLevel(int EXP,int Level,int Stage,int Mode,int ttime,int percento,int stone,int wlevel,int gotEXP,int gotStone,int EggHealth, int original){
	...
	if(EXP>=Limit) {
		...
		Level++;
		printf("          Level Up  %d --> %d\n\n",Level-1,Level);
		PlaySound("Level Up.wav", NULL, SND_ASYNC | SND_ALIAS);
	}
